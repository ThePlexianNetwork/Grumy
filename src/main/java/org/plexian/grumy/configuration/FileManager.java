...
import main.java.org.plexian.grumy.world.Chunk;
import main.java.org.plexian.grumy.world.World;
import main.java.org.plexian.grumy.world.Chunk.ChunkState;
...
public class FileManager {
    ...
    public FileManager(World w) {
        /**
         * This just sets up the game directory to be at the user's home
         * directory, in a folder called Pendulum. On Mac OSX the home directory
         * is /Users/[Username], on Linux it is /home/[Username].
         */
    ...
    public void updateWorldName(String name) {
        /**
         * This just sets up the current world directory to be inside the
         * Pendulum folder, in a folder titled by the world's name.
         */
    ...
    public void saveChunk(Chunk c) {
        ...
        /**
         * We use a try-catch block to ensure that any error's are properly
         * dealt with.
         */
        try {
            /**
             * This simply creates a new file to save the chunk to. It is
             * located inside the current world directory, with a filename of
             * x-y where x=c.position.x / 16, and y=c.position.y / 16.
             */
            ...
            /**
             * Deletes the file if it exists to ensure that we only have the
             * latest copy of the chunk saved.
             */
            outDir.delete();
            ...
            /**
             * We loop through all the tiles in the chunk, and write their id's
             * to the file.
             */
            ...
            /**
             * If there was any error (probably an I/O error), print out the
             * cause.
             */
            e.printStackTrace();
        }
    ...
    public Chunk loadChunk(int x, int y) {
        /**
         * We create a new instance of File, located at the current world
         * directory with filename: x-y.chk where x=chunk.position.x/16,
         * y=chunk.position.y/16
         */
        ...
        /**
         * While this seems kind of pointless, its actually not. Trust me. The
         * reason we don't initialize it outside the try loop is because it
         * throws an error, and we want to handle that in case it does happen.
         */
        ...
        try {
            ...
            /**
             * We loop through the number of tiles in a chunk, essentially
             * casting a new chunk from the file. I'm feeling like a
             * blacksmith!!
             */
            for (int x1 = 0; x1 < Game.CHUNK_SIZE; x1++) {
                for (int y1 = 0; y1 < Game.CHUNK_SIZE; y1++) {
                    /**
                     * We read the tile id in from the file and store it as an
                     * integer.
                     */
                    int i = in.readInt();

                    /**
                     * Set the tile in the chunk at (x, y) to the read-in id.
                     * Nifty huh?
                     */
                    c.setTile(x1, y1, i);
                }
            ...
            /**
             * We need to rebuild the chunk's display list for our changes to
             * take effect in the rendering aspect, so thats what we do.
             */
            c.rebuild();

            /**
             * We close the file to ensure quality security. Our #1 goal here at
             * Oscorp.
             */
            in.close();
        } catch (Exception e) {
            /**
             * If there was an exception, it was caused by the FileReader
             * hitting the end of the file, so we just ignore it.
             */
        }
